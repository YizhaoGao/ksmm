#!/usr/bin/env python3
"""
Training script for MonarchLinear to approximate a given weight matrix.

This script trains a MonarchLinear layer to approximate a dense weight matrix
...
# Import MonarchLinear
from src.models.layers.monarch_linear import MonarchLinear


def parse_args():
    """Parse command line arguments."""
    parser = argparse.ArgumentParser(description='Train MonarchLinear to approximate dense weights')
    
    # Model parameters
    parser.add_argument('--weight_path', type=str, required=True,
                        help='Path to the .pth file containing the dense weight matrix')
    parser.add_argument('--nblocks', type=int, default=4,
                        help='Number of blocks for MonarchLinear (default: 4)')
    ...
    # Output parameters
    parser.add_argument('--output_dir', type=str, default='./trained_models_monarch',
                        help='Directory to save trained models (default: ./trained_models_monarch)')
    parser.add_argument('--save_prefix', type=str, default='monarch',
                        help='Prefix for saved model files (default: monarch)')
...
def compute_target_outputs(dense_weight: torch.Tensor, input_batch: torch.Tensor):
...
def compute_grad_norm(model: nn.Module):
    ...
    # For MonarchLinear, compute individual parameter gradient norms with names
...
def calculate_compression_stats(dense_weight: torch.Tensor, monarch_layer: MonarchLinear):
    """Calculate compression statistics."""
    dense_params = dense_weight.numel()
    sparse_params = sum(p.numel() for p in monarch_layer.parameters())
    ...
    return {
        ...
        'saving': monarch_layer.saving
    }


def evaluate_model(model: MonarchLinear, val_loader: DataLoader, dense_weight: torch.Tensor, 
    ...
    with torch.no_grad():
        for batch_idx, (inputs,) in enumerate(val_loader):
            ...
            targets = compute_target_outputs(dense_weight, inputs)
            
            # Forward pass through MonarchLinear
...
def save_model_and_stats(model: MonarchLinear, optimizer: optim.Optimizer, scheduler, epoch: int, 
    ...
    checkpoint = {
        ...
        'in_features_extended': model.in_features_extended,
        'out_features_extended': model.out_features_extended,
        'nblocks': getattr(model, 'nblocks', None),
...
def save_loss_curves(train_losses: list, val_losses: list, epochs: list, nblocks: int, output_dir: str, save_prefix: str):
    ...
    json_path = os.path.join(output_dir, f"{save_prefix}_nblocks_{nblocks}_loss_curves.json")
    with open(json_path, 'w') as f:
        json.dump(loss_data, f, indent=2)
    
    # Save to CSV for easy plotting
    csv_path = os.path.join(output_dir, f"{save_prefix}_nblocks_{nblocks}_loss_curves.csv")
    import csv
    with open(csv_path, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(['epoch', 'train_loss', 'val_loss', 'nblocks'])
...
def main():
    ...
    # Create MonarchLinear layer
    print(f"\nCreating MonarchLinear for shape [{out_features}, {in_features}] with nblocks {args.nblocks}")
    ...
    # Calculate compression statistics
    compression_stats = calculate_compression_stats(dense_weight, monarch_layer)
    ...
    print(f"  MonarchLinear saving: {compression_stats['saving']:.4f}")
    ...
    train_loader = generate_random_data(
        in_features, num_train_samples, args.batch_size, dtype, device
    )
    val_loader = generate_random_data(
        in_features, num_val_samples, args.batch_size, dtype, device
    )
    ...
    optimizer = create_optimizer(monarch_layer, args.optimizer, args.lr, args.weight_decay)
    ...
    for epoch in range(1, args.num_epochs + 1):
        ...
        for batch_idx, (inputs,) in enumerate(progress_bar):
            ...
            grad_norm, weight_grad_norms, batch_weight_names = compute_grad_norm(monarch_layer)
            ...
            if args.max_grad_norm < float('inf'):
                torch.nn.utils.clip_grad_norm_(monarch_layer.parameters(), args.max_grad_norm)
        ...
        # Validation phase
        avg_val_loss = evaluate_model(monarch_layer, val_loader, dense_weight, criterion, device)
        ...
        if avg_val_loss < best_val_loss:
            ...
            save_model_and_stats(
                monarch_layer, optimizer, scheduler, epoch, avg_train_loss, avg_val_loss,
    ...
    # Save final model
    save_model_and_stats(
        monarch_layer, optimizer, scheduler, args.num_epochs, avg_train_loss, avg_val_loss,
        compression_stats, args.output_dir, f"{args.save_prefix}_final", 
        avg_grad_norm, avg_weight_grad_norms, weight_names
    )
    
    # Save loss curves for nblocks comparison study
    save_loss_curves(train_losses, val_losses, epochs, args.nblocks, args.output_dir, args.save_prefix)
