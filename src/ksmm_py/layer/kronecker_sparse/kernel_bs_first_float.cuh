...
void best_kernel_bs_first_float(float *input, float *values, float *output, int batch_size, int a, int b, int c, int d, dim3 &blockGrid, dim3 &threadsPerBlock){
	while (1) {
		...
		if (batch_size == 25088 && a == 32 && b == 64 && c == 64 && d == 1) {
			...
			blockGrid.x = 32;
			blockGrid.y = 98;
			kernel_bs_first_float<float4, 64, 32, 256, 16, 16, 4, true, 4><<<blockGrid, threadsPerBlock>>>(input, values, batch_size, output, a, b, c, d);
			break;
		}
		if (batch_size == 25088 && a == 1 && b == 512 && c == 512 && d == 128) {
			...
			blockGrid.x = 1024;
			blockGrid.y = 392;
			kernel_bs_first_float<float4, 64, 16, 64, 4, 8, 4, true, 16><<<blockGrid, threadsPerBlock>>>(input, values, batch_size, output, a, b, c, d);
			break;
		}
		if (batch_size == 25088 && a == 1 && b == 512 && c == 512 && d == 1) {
			...
			blockGrid.x = 8;
			blockGrid.y = 784;
			kernel_bs_first_float<float4, 64, 16, 32, 4, 8, 4, true, 16><<<blockGrid, threadsPerBlock>>>(input, values, batch_size, output, a, b, c, d);
			break;
		}
