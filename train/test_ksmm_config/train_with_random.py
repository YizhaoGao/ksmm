...
# Import KS modules
from ksmm_triton.ksmm_module import create_butterfly_chain, KSLinearTriton, create_given_pattern_ks_layer
...
def parse_args():
    ...
    parser.add_argument('--patterns', type=str, required=True,
                        help='List of patterns for KS layers, e.g., "[(6,64,64,1),(6,64,64,1)]"')
...
def save_loss_curves(train_losses: list, val_losses: list, epochs: list, patterns: str, output_dir: str, save_prefix: str):
    ...
    json_path = os.path.join(output_dir, f"{save_prefix}_patterns_{patterns}_loss_curves.json")
    with open(json_path, 'w') as f:
        json.dump(loss_data, f, indent=2)
    
    # Save to CSV for easy plotting
    csv_path = os.path.join(output_dir, f"{save_prefix}_patterns_{patterns}_loss_curves.csv")
    import csv
    with open(csv_path, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(['epoch', 'train_loss', 'val_loss', 'patterns'])
...
def main():
    ...
    ks_chain = create_given_pattern_ks_layer(
        in_features,
        out_features,
        patterns=patterns,
        dtype=dtype,
    ...
    save_loss_curves(train_losses, val_losses, epochs, args.patterns, args.output_dir, args.save_prefix)
